<?php
    ...
    $datos_query = "SELECT * FROM wishlist INNER JOIN pelicula ON wishlist.id_pelicula = pelicula.id AND id_usuario = $id_user_actual";

    $datos_query_total = "SELECT COUNT(id_usuario) FROM wishlist";
    ...
    if($total >= 1 && $pagina <= $N_pages){
        ...
        foreach($datos as $rows){
            $tabla.='
                <tr class="has-text-centered" >
                    ...
                    <td>'.$rows['nombre'].'</td>
                    <td>
                        <a href="index.php?vista=wishlist_quitar&pelicula_id_wishlist='.$rows['id'].'" class="button is-warning is-rounded is-small">Quitar de la wishlist</a>
                    </td>
                </tr>
            ';
            $cont++;
        ...
        if($total >= 1){
            ...
            <tr class="has-text-centered" >
                <td colspan="7">
                    No hay peliculas en la wishlist
                </td>
            </tr>
